...
export default function AddOurTeamModal(props) {
  ...
  const [editedName, setEditedName] = useState(props.filterListData.StaffName);
  const [editedPosition, setEditedPosition] = useState(
    props.filterListData.StaffPosition
  );
  const [isLive, setIsLive] = useState(
    props.filterListData.IsLive === "1" ? true : false
  );
  const [email, setEmail] = useState(props.filterListData.StaffEmail);
  const [mobile, setMobile] = useState(props.filterListData.StaffMobile);
  const [editedImage, setEditedImage] = useState(
    props.filterListData.StaffImage
  );
  ...
  //     setEditedImage(props.filterListData.StaffImage);
  //     setErrorMessage("");
  //     return;
  //   }
  ...
  const handleImageChange = (e) => {
    ...
    // If file is valid, set it for upload and preview
    const reader = new FileReader();
    reader.onloadend = () => {
      setImagePreview(reader.result);
    };
      ...
      >
        ...
        <form
          onSubmit={(e) => {
            ...
            props.UpdateStaff(
              props.filterListData.StaffUUID,
              editedName,
              mobile,
              email,
              editedImage,
