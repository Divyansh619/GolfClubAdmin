...
const Restaurant = ({
  ...
  const handleFileChange = (e) => {
    ...
    // Set the file for upload and preview
    setSelectedFile(file);
    const reader = new FileReader();
    reader.onloadend = () => {
      setPreviewImage(reader.result);
  ...
  const uploadImage = () => {
    ...
    formdata.append("SubSectionName", subMenu);
  ...
  return (
    <div>
      ...
      {openEditSection && (
        // <div className="flex border py-4 border-[#34BE82] rounded-lg  items-center justify-evenly  ">
        //   <label
        //     htmlFor="file-upload"
        //     className="relative  cursor-pointer bg-white rounded-md font-medium shadow-md py-2 px-3 border border-gray-300 hover:border-gray-400 text-xs"
        //   >
        ...
        //         src={previewImage}
        //         alt="Preview"
        //         className="w-50 h-50 object-contain rounded-md shadow-md"
        //       />
        //     </div>
        //   )}
        // </div>
